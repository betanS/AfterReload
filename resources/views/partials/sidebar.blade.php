<aside id="main-sidebar" class="fixed inset-y-0 left-0 z-50 w-64 bg-[#1b1b1b] border-r border-[#222222] transform -translate-x-full lg:translate-x-0 lg:static lg:block transition-transform duration-300 ease-in-out shrink-0">
    <div class="flex flex-col h-full">
        ...
        <nav class="flex-1 px-4 py-8 space-y-2 overflow-y-auto">
            <a href="{{ route('servers.index') }}" data-spa-link class="flex items-center gap-4 px-4 py-3 rounded-sm font-bold text-sm transition group {{ request()->routeIs('servers.*') ? 'bg-[#5b7cff] text-white' : 'text-slate-400 hover:bg-[#222222] hover:text-white' }}">
            ...
            <a href="{{ route('ranking') }}" data-spa-link class="flex items-center gap-4 px-4 py-3 rounded-sm font-bold text-sm transition group {{ request()->routeIs('ranking') ? 'bg-[#5b7cff] text-white' : 'text-slate-400 hover:bg-[#222222] hover:text-white' }}">
            ...
            @if(auth()->check() && auth()->user()->isAdmin())
                ...
                <a href="{{ route('admin.index') }}" data-spa-link class="flex items-center gap-4 px-4 py-3 rounded-sm font-bold text-sm transition group {{ request()->routeIs('admin.*') ? 'bg-[#5b7cff] text-white' : 'text-[#5b7cff] hover:bg-[#5b7cff] hover:text-white' }}">
